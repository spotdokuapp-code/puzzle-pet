...
**Interfaces:**
- Consumes: `PPBond.award`, `PPBond.claimVisit`, `PPBond.claimPet`, `PPBond.levelFor` (Tasks 1–3); bond DOM ids (Task 5).
- Produces:
...
- [ ] **Step 6: Claim the daily visit at boot**
...
```js
  ...
  if (S.pet.species) {
    ...
    if (visit) log('bond_visit', { xp: visit.gained });
    applyBond(visit);
  }
```

The `S.pet.species` guard means a brand-new player claims their first visit only after onboarding completes, not while staring at the welcome screen.
