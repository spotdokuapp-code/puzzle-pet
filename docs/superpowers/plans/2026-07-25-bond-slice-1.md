...
Reviewed during execution and triaged as correct to defer. Recorded here because they are easy to "clean up" wrongly later.

- **The stale `puzzlepet.v1` key is left in place deliberately.** `save()` swallows quota failures, so `migrate()` cannot prove the v2 write landed. Keeping v1 is what makes the re-migrate path safe on the next boot. Do not delete it as dead data. *(This is the one item worth a code comment in `storage.js` that it does not yet have.)*
- `thresholdFor` recomputes the endless tail on every call, so `levelFor` is O(n²) in level number — roughly 1275 iterations at level 50, a few calls per render. Never observable.
- `thresholdFor(level < 1)` reads `L[-1]` and throws. Unreachable: `levelFor` starts at 1 and every caller passes ≥ 1.
- `speech.js` holds `lastText` at module scope, so anti-repeat is global rather than per-caller. The practical effect — the home screen and pet room never show the same line back to back — is better than per-caller behavior, not worse.
- The endless-tier coin gift pays once per award that crosses a boundary, not once per level crossed. Unreachable: the largest single award is 10 XP and the smallest endless step is ~575, and `_grantXp` bypasses `applyBond` entirely.
- The `if (screen-pet active)` re-render branch in `rename-save` is currently unreachable — `btn-settings` exists only on the home screen. It becomes correct the moment Settings is exposed elsewhere.
- The speech denylist contains both `cry` and `crying`; the first already substring-matches. Redundant but self-documenting.
